...
SVM.prototype.train = function(features, labels) {
  ...
  if (this.options.whitening) {
    ...
    for (var j = 0; j < this.D; j++) {
      ...
      for (i = 0; i < this.N; i++) {
        this.X[i][j] =
          (features[i][j] - this.minMax[j].min) /
          (this.minMax[j].max - this.minMax[j].min);
      }
    }
  } else {
    this.X = features;
  ...
  while (passes < this.options.maxPasses && iter < this.options.maxIterations) {
    ...
    for (i = 0; i < m; i++) {
      ...
      if (
        (labels[i] * Ei < -this.options.tol && alpha[i] < this.options.C) ||
        (labels[i] * Ei > this.options.tol && alpha[i] > 0)
      ) {
        j = i;
        while (j === i) {
        ...
        b1 =
          this.b -
          Ei -
          labels[i] * (alpha[i] - ai) * kernel[i][i] -
          labels[j] * (alpha[j] - aj) * kernel[i][j];
        b2 =
          this.b -
          Ej -
          labels[i] * (alpha[i] - ai) * kernel[i][j] -
          labels[j] * (alpha[j] - aj) * kernel[j][j];
        this.b = (b1 + b2) / 2;
  ...
  if (this.options.kernel === 'linear' && this.W) {
    ...
    for (i = 0; i < this.N; i++) {
      ans +=
        this.alphas[i] *
        this.Y[i] *
        this.kernel.compute([features], [this.X[i]])[0][0];
    }
  }
  return ans;
};
...
SVM.prototype.supportVectors = function() {
  if (!this._trained && !this._loaded) {
    throw new Error(
      'Cannot get support vectors, you need to train the SVM first'
    );
  }
  if (this._loaded && this.options.kernel === 'linear') {
    throw new Error(
      'Cannot get support vectors from saved linear model, you need to train the SVM to have them'
    );
  }
